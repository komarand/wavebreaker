...
Goal: introduce the Kaggle EDA Engine as a separate data-evidence layer without breaking the existing research/reasoning pipeline.

Includes tasks: `28`–`35`.

This milestone creates the EDA package, schemas, config, artifact layout, dataset IO abstractions, and offline fixtures. It does not yet run full EDA modules on real Kaggle data.
