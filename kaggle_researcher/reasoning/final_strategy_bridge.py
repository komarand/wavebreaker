...
def freeze_strategy_selection(
    ...
    *,
    ...
    core = preferred[: budget.max_core_experiments]
    backlog = preferred[budget.max_core_experiments:][: limits["max_backlog_experiments"]]
    ...
    source_links, eda_links = _compile_provenance(
        synthesis_context, actions, allowed
    )
    action_provenance = [
        ActionProvenance(
...
def skeleton_to_result(
    ...
    *,
    ...
    payload = {
        ...
        "feature_experiment_families": skeleton.feature_experiment_families,
        "core_experiments": skeleton.core_experiments,
        "experiment_backlog": skeleton.experiment_backlog,
        "experiments": [*skeleton.core_experiments, *skeleton.experiment_backlog],
        "experiment_budget": skeleton.experiment_budget,
        "dependency_graph": skeleton.dependency_graph,
        "sections": skeleton.section_structure,
        "evidence_catalog": skeleton.evidence_catalog,
        "source_to_hypothesis_links": skeleton.source_to_hypothesis_links,
        "hypothesis_to_eda_links": skeleton.hypothesis_to_eda_links,
        "action_provenance": skeleton.action_provenance,
...
def validate_rendering_draft(
    ...
    for label in expected:
        if expected[label] != actual[label]:
            raise StrategyBridgeError(
                f"rendering {label} IDs differ from frozen skeleton: expected={sorted(expected[label])}, actual={sorted(actual[label])}"
            )
...
def _compile_provenance(context: FinalSynthesisContext, actions: list[FinalStrategyAction], allowed: Mapping[str, Any]) -> tuple[list[SourceToHypothesisLink], list[HypothesisToEdaLink]]:
    source_links = []
    eda_links = []
    used_hypotheses = {str(hypothesis) for action in actions for hypothesis in action.hypothesis_ids}
...
def _compile_arm(arm: Any, family_id: str) -> ExperimentArm:
    return ExperimentArm(
        ...
        leakage_risk=arm.leakage_risk, dependencies=arm.dependencies,
    )
