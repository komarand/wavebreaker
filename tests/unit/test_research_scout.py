...
def test_scout_mode_writes_artifacts_and_skips_docx(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert (run_path / "research_scout_raw_output.json").is_file()
    assert (run_path / "research_scout_canonical_output.json").is_file()
    assert (run_path / "research_to_eda_validation_errors.json").is_file()
